...
int main(int argc, char **argv)
{
    ...
    {
        printf("Errore nel numero dei parametri dato che argc=%d (bisogna passare almeno tre nomi di file!)\n", argc);
        exit(1);
    }
    /* Calcoliamo il numero di file passati e quindi di figli che dovranno essere creati */
    N = argc - 1;
    ...
    for (i=0; i < N; i++)
    {
        ...
        if (pid == 0)
        {
            ...
            if (pid == 0)
            {
                /* codice del nipote */
                /* in caso di errori nei figli o nei nipoti decidiamo di tornare -1 che corrispondera' al valore 255 che non puo' essere un valore accettabile di ritorno */
                /* printf("DEBUG-Sono il processo nipote del figlio di indice %d e pid %d sto per eseguire il comando sort -r/-f per il file %s\n", i, getpid(), argv[(i*2)+1]); */
            ...
            close(p[1]);
            /* adesso il figlio legge dalla pipe fino a che ci sono caratteri e cioe' linee inviate dal nipote tramite il comando sort */
            ...
            while (read(p[0], &(S.linea_letta[j]), 1))
            {
                ...
                {       
			        ...
			        S.lung_linea=j; /* la lunghezza della linea NON compreso il terminatore e' j (se fosse richiesta la lunghezza della linea compreso il terminatore, sarebbe j+1) */	
                    /* nell'array linea_letta, al termine della lettura, avremo l'ultima linea ricevuta */
			...
			/* il figlio ritorna la lunghezza della linea inviata compreso il terminatore (se fosse NON compreso il terminatore NON andrebbe incrementato) */
