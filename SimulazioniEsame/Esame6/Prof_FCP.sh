...
for file1 in `cat  /tmp/nomiAssoluti$$-1`; do	# per ogni file trovato nella prima gerarchia
	n=1 	# idem come prima: ATTENZIONE QUESTA INIZIALIZZAZIONE E' DA FARE ALL'INTERNO DI QUESTO CICLO!
	for i; do	# per ognuna delle gerarchie
		if test $n -gt 1; then	#saltiamo il file temporaneo della prima gerarchia; N.B. in alternativa prima di iniziare questa parte di confronto si poteva usare il comando shift e quindi avere in $* solo le gerarchie a partire dalla G2	
			for file2 in `cat  /tmp/nomiAssoluti$$-$n`; do
				# Stampa di DEBUG
